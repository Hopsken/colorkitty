...
export class PaletteComponent extends React.PureComponent<Props> {
  renderColor = (color: string, index: number) => {
    return (
      ...
      >
        <span onClick={this.handleClick.bind(this, color)}>
  ...
  renderBottom = () => {
    ...
    const Stars = (
      <Button className={styles['bottom-fav']} onClick={this.handleClickLike}>
        <Icon type="heart" theme="filled" className={cx({ active: liked })} />
    ...
    const DeleteBtn = (
      <Popconfirm
        ...
        cancelText="No"
      >
        <Icon type="delete" style={{ color: '#a6a6a6', marginLeft: 8 }} />
      </Popconfirm>
    )
    ...
    return (
      <div className={styles.bottom}>
        <div className={styles['bottom-info']}>
          ...
          {creator && (
            <span className={styles['bottom-meta']}>by {creator}</span>
          )}
        </div>
        {Stars}
  ...
  render() {
    ...
    return (
      <div className={cx({ palette: true, [className!]: !!className })}>
        <div className={styles.colors}>{colorCards}</div>
